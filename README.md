...
### Parameters
- `-List`: Path to the raw list of installed apps on the phone ( > [!CAUTION] > NOTICE: if not specified, the default list is used)
- `-Dictionary`: Path to the dictionary that associated images with package names ( > [!CAUTION] > NOTICE: if not specified, the default dictionary is used)
- `-IconBank`: Path to the folder containing the icons on which the program will search for the icons 
to use ( > [!CAUTION] > NOTICE: if not specified, the default icon bank is used)
...
- `-Default`: Whether to use default images or user images ( > [!CAUTION] > NOTICE: if true, the other parameters are ignored, and the default icon pack is created, intended for tests, not actual use)